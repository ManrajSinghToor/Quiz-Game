...
const buildUserPayload = (user) => ({
  ...
  lastDailyQuizDate: user.lastDailyQuizDate || null,
  lastDailyQuizResult: user.lastDailyQuizResult || null,
});
...
export const register = async (req, res) => {
  try {
    ...
    const usernameExists = await User.findOne({ name: new RegExp(`^${cleanName}$`, "i") });
    if (usernameExists) {
...
export const login = async (req, res) => {
  try {
    ...
    // Allow user to log in via Email or Username
...
export const saveQuizResult = async (req, res) => {
  try {
    ...
    if (mode === "daily") {
      setUpdates.lastDailyQuizDate = localDate;
      setUpdates.lastDailyQuizResult = {
        date: localDate,
        score: incScore,
        accuracy: incTotalQuestions > 0 ? Math.round((incCorrect / incTotalQuestions) * 100) : 0,
    ...
    const stats = {
      ...
      accuracy: nextAccuracy,
      currentStreak: nextStreak,
      lastDailyQuizDate: updatedUser.lastDailyQuizDate,
      lastDailyQuizResult: updatedUser.lastDailyQuizResult,
    };
