...
import { BookOpen, Calendar, Swords, Zap, Trophy, Target, Flame, Award, Users } from "lucide-react";
...
const Dashboard = () => {
  ...
  const stats = [
    { label: "Total Score", value: (profileStats.totalScore || 0).toLocaleString(), icon: Trophy },
    { label: "Quizzes Played", value: String(profileStats.totalQuizzes || 0), icon: Target },
    { label: "Questions Answered", value: String(profileStats.totalQuestions || 0), icon: BookOpen },
    { label: "Current Streak", value: `${profileStats.currentStreak || 0} day${(profileStats.currentStreak || 0) === 1 ? "" : "s"}`, icon: Flame },
    { label: "Win Rate", value: `${profileStats.accuracy || 0}%`, icon: Award },
  ];
  ...
  return (
    <div className="page">
      ...
      <main className="container" style={{ paddingTop: "2rem", paddingBottom: "2rem" }}>
        <div className="dashboard-welcome animate-fade-in" style={{ marginBottom: "2rem", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: "1rem" }}>
          <div>
            <h1>Welcome back, {displayName}! 👋</h1>
            <p>Ready to test your knowledge today?</p>
          </div>
          <div className="online-indicator" style={{ padding: "0.5rem 1rem", fontSize: "0.9rem" }}>
            <div className="pulse-dot" />
            <Users style={{ width: "1rem", height: "1rem" }} />
            <span style={{ fontWeight: 700 }}>{onlineCount} Player{onlineCount === 1 ? "" : "s"} Online</span>
          </div>
        </div>
