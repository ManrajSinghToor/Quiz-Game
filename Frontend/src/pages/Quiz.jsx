...
import { saveDailyChallengeCompletionState, getTodayDailySubject } from "@/data/dailyChallengeData";
...
const Quiz = () => {
  ...
  useEffect(() => {
    ...
    const saveResult = async () => {
      ...
      try {
        const apiBase = getApiBase();
        const res = await fetch(`${apiBase}/api/auth/quiz-result`, {
          method: "POST",
          headers: {
        ...
        if (resData.stats) {
          updateUser(resData.stats);
          if (socket) {
            socket.emit("stats:updated", resData.stats);
        ...
        if (mode === "daily") {
          ...
          saveDailyChallengeCompletionState(score, accuracy, totalTime, userKey);
        }
      } catch (error) {
  ...
  useEffect(() => {
    if (mode === "daily") {
      const todaySubj = getTodayDailySubject();
      if (todaySubj && todaySubj.name) {
        setSelectedSubject(todaySubj.name);
      }
    }
  }, [mode]);

  const startQuiz = async (subjectOverride) => {
    const todaySubjName = getTodayDailySubject().name;
    const targetSubject = (typeof subjectOverride === "string" && subjectOverride)
      ? subjectOverride
      : (mode === "daily" ? todaySubjName : selectedSubject);
    ...
    try {
      const fetchedQuestions = await fetchQuestionsFromAPI({
        ...
        classLevel,
        questionCount: mode === "daily" ? 10 : questionCount,
        topics: selectedTopics,
      });
    ...
    return (
      <div className="page" style={{ minHeight: "100vh" }}>
        ...
        <QuizSetup
          ...
          onSelectAllTopics={(topics) => setSelectedTopics(topics)}
